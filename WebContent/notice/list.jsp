...
if (what == null || query == null) {
	...
	rs = st.executeQuery(
	"select count(*) as cnt from (select * from (select @rownum:=@rownum+1 as num,n. *from (select *from notice order by regfate) n, (select @rownum:=0) r) b)c");
	rs.next();
	cnt = rs.getInt("cnt");
	rs = st.executeQuery(
	"select * from (select * from (select @rownum:=@rownum+1 as num,n. *from (select *from notice order by regfate) n, (select @rownum:=0) r) b)c where num between 1 and 5");
} else {
	if (what.equals("")) {
		what = "title";
	}
	PreparedStatement st2 = con.prepareStatement(
	"select*from(select count(*) as cnt from (select * from (select @rownum:=@rownum+1 as num,n. *from (select *from notice order by regfate) n, (select @rownum:=0) r)c where "
			+ what + " like ?)d)e");
	...
	"select*from(select @rownum2:=@rownum2+1 as num2,d.* from (select * from (select @rownum:=@rownum+1 as num,n. *from (select *from notice order by regfate) n, (select @rownum:=0) r)c where "
						...
						<tbody>
							...
							<tr>
								...
								<td class="title indent text-align-left"><a
									href="/NoticeDetailController?id=<%=rs.getString("id")%>"><%=rs.getString("TITLE")%></a></td>
